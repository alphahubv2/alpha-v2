...
$wdLines = @(
    ...
    "        ' No miner â€” start with target config",
...
# ---- START (watchdog only â€” it starts exactly 1 miner) ----
...
if ($proc -and $minerCount -eq 1) {
    Show "OK" "Miner RUNNING â€” exactly 1 instance (PID: $($proc.Id), RAM: $([math]::Round($proc.WorkingSet64/1MB,0)) MB)"
} elseif ($minerCount -gt 1) {
    Show "!!" "Multiple miners detected ($minerCount) â€” watchdog will fix this in 30s"
} else {
...
$wdCount = @($wd).Count
if ($wdCount -eq 1) { Show "OK" "Watchdog ACTIVE â€” exactly 1 instance (PID: $($wd.ProcessId))" }
elseif ($wdCount -gt 1) { Show "!!" "Multiple watchdogs ($wdCount) â€” will self-correct" }
